...
test('#9 员工模式:非法role坏计划仍带问题回喂重拆一次(rmOk失败)', async () => {
  ...
  const leafIds = []; const walk = (a) => (a || []).forEach((s) => s.body ? walk(s.body) : leafIds.push(s.id)); walk(plan.steps); // 开发步折叠进 dev_session,遍历 body
  assert.deepEqual([...new Set(leafIds)].sort(), ['a', 'b']);
});
