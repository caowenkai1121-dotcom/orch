...
test('调度提示含复杂度路由(简单直接/复杂开需求会议)+按人聚合拆步', async () => {
  ...
  assert.match(seen, /开发步按阶段/);        // 拆步哲学:按阶段拆开发步(系统折叠成一个 CLI 会话)
  assert.match(seen, /独立成步且换人把关/);  // 测试/验收独立 CLI + 换人
  seen = '';
...
test('业务对象管理系统属于复杂交付,员工规划失败也必须多员工会议保底', async () => {
  ...
  const leaf = []; const walkLeaf = (a) => (a || []).forEach((s) => s.body ? walkLeaf(s.body) : leaf.push(s.id)); walkLeaf(plan.steps); // 开发步折叠进 dev_session,遍历 body
  assert.ok(leaf.includes('publish_manifest'), '复杂业务系统应有发布清单步骤(可在开发会话内)');
});
...
test('显式前后端技术栈的网站开发必须复杂编排并列清技术架构', async () => {
  ...
  // 开发步(backend_impl/frontend_impl/publish_manifest)折叠进 dev_session,验收独立;body 保留原开发阶段
  assert.deepEqual(workIds, ['dev_session', 'acceptance_test']);
  const sess = plan.steps.find((s) => s.id === 'dev_session');
  assert.deepEqual((sess.body || []).map((b) => b.id), ['backend_impl', 'frontend_impl', 'publish_manifest']);
...
test('复杂应用规划带交付蓝图和计划健康评分', async () => {
  const roles = [
    { id: 'product-manager', dept: 'product', name: '产品经理', description: '需求范围与业务流程', prompt: 'p', executor: 'claude' },
    { id: 'engineering-backend-architect', dept: 'engineering', name: '后端架构师', description: 'Java Spring Boot 接口和数据库', prompt: 'p', executor: 'claude' },
    { id: 'engineering-frontend-developer', dept: 'engineering', name: '前端开发', description: 'Vue 页面和组件', prompt: 'p', executor: 'claude' },
    { id: 'testing-api-tester', dept: 'testing', name: 'API测试员', description: '接口和验收测试', prompt: 'p', executor: 'codex' },
  ];
  const depts = [
    { id: 'product', name: '产品部' },
  ...
  const claude = { async run() { throw new Error('模拟员工LLM规划失败'); } };
  const plan = await makePlan('前端使用 Vue 后端使用 Java SpringBoot 开发一个 DMS 供应商管理系统，并发布到应用广场', { mode: 'llm', agents: ['claude', 'codex'], roles, depts, refine: true, templatesDir: __dirname, claude });
  const text = JSON.stringify(plan);

  assert.ok(plan.delivery_blueprint);
  assert.deepEqual(plan.delivery_blueprint.sections, ['技术架构', '目录结构', 'API清单', '数据模型', '页面清单', '部署发布', '测试验收', '风险边界']);
  ...
  assert.ok(plan.diagnostics.score >= 80);
  assert.ok(plan.validation.ok);
  assert.ok(plan.planning_stats.quality_score >= 80);
});
