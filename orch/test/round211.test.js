...
test('foldDevSession:全部开发步折叠成一个 session,测试/验收/门排除且下游依赖重指向', () => {
  const planner = require('../planner');
  const roleMap = {
    dev: { id: 'dev', dept: 'eng', name: '后端', description: '实现' },
    fe: { id: 'fe', dept: 'eng', name: '前端', description: '页面' },
    qa: { id: 'qa', dept: 'testing', name: '验收员', description: '测试验收' },
  };
  const plan = { task: 't', steps: [
    { id: 'impl_backend', role: 'dev', agent: 'claude', prompt: '后端', deps: ['decide_plan'] },
    { id: 'impl_frontend', role: 'fe', agent: 'claude', prompt: '前端', deps: ['impl_backend'] },
    { id: 'acceptance', role: 'qa', agent: 'codex', prompt: '独立验收', deps: ['impl_frontend'] },
  ], meeting: { meetIds: [], decideId: 'decide_plan' } };
  planner.foldDevSession(plan, roleMap);
  const ids = plan.steps.map((s) => s.id);
  assert.ok(ids.includes('dev_session'), '开发步应折叠成 dev_session, 实际=' + ids.join(','));
  assert.ok(!ids.includes('impl_backend') && !ids.includes('impl_frontend'), '开发步不再是顶层');
  assert.ok(ids.includes('acceptance'), '验收步保持独立');
  const sess = plan.steps.find((s) => s.id === 'dev_session');
  assert.equal(sess.type, 'session');
  assert.equal(sess.body.length, 2, 'body 含 2 个开发阶段');
  assert.deepEqual(sess.deps, ['decide_plan'], 'session 继承首个开发步的外部依赖');
  const acc = plan.steps.find((s) => s.id === 'acceptance');
  assert.deepEqual(acc.deps, ['dev_session'], '验收依赖重指向 session, 实际=' + acc.deps);
});

test('isTestish:验收/测试/门步识别,纯开发步不误判', () => {
  const planner = require('../planner');
  const rm = { qa: { name: '验收员', dept: 'testing' }, dev: { name: '后端', dept: 'eng' } };
  assert.ok(planner.isTestish({ id: 'acceptance_test', role: 'qa' }, rm));
  assert.ok(planner.isTestish({ id: 'gate', isGate: true }, rm));
  assert.ok(planner.isTestish({ id: 'q', type: 'loop' }, rm));
  assert.ok(!planner.isTestish({ id: 'impl_backend', role: 'dev' }, rm), '纯开发步不是测试步');
});

test('runSession:多阶段在一次 adapter.run 里跑完(一个 CLI 会话),阶段节点点亮', async () => {
  const engine = require('../engine');
  let runs = 0; let gotPrompt = '';
  const adapter = { async run({ prompt, timeoutScale }) { runs++; gotPrompt = prompt; assert.equal(timeoutScale, 2, '超时按阶段数放大'); return { output: '【阶段完成:s1】\n【阶段完成:s2】\n done', success: true }; } };
  const statuses = {};
  const ctx = { adapters: { claude: adapter }, workspace: { make: async () => '.' }, onStatus: (id, v) => { statuses[id] = v; }, onLog: () => {}, onResult: () => {} };
  const step = { id: 'dev_session', type: 'session', agent: 'claude', body: [
    { id: 's1', agent: 'claude', prompt: '阶段一实现' },
    { id: 's2', agent: 'claude', prompt: '阶段二实现' },
  ] };
  const res = await engine.runSession(step, ctx, '');
  assert.equal(runs, 1, '两个开发阶段只 spawn 一次 CLI(同一会话), 实际 run=' + runs);
  assert.ok(res.success);
  assert.ok(/阶段 1\/2/.test(gotPrompt) && /阶段 2\/2/.test(gotPrompt), 'prompt 含各阶段');
  assert.equal(statuses.s1, 'done'); assert.equal(statuses.s2, 'done');
});

test('聚合拆解:complexPlanSufficient 2步2人即充分;prependMeeting 复杂判定即开会不看步数', () => {
  const planner = require('../planner');
  const roleMap = {
    'chief-orchestrator': { id: 'chief-orchestrator', dept: '__meta', name: '总调度', description: '主持' },
    dev: { id: 'dev', dept: 'eng', name: '开发', description: '实现' },
    qa: { id: 'qa', dept: 'testing', name: '验收', description: '测试' },
    ux: { id: 'ux', dept: 'design', name: '设计', description: '界面' },
  };
  // 实现聚合一步 + 独立验收一步 = 合格复杂计划(不再要求 4 步)
  const p1 = { task: '开发工具', steps: [
    { id: 'impl_all', role: 'dev', prompt: '按清单实现全部功能:①增删 ②存储 ③筛选,逐项自测', deps: [] },
    { id: 'acceptance', role: 'qa', prompt: '独立验收全部功能点', deps: ['impl_all'] },
  ] };
  const planner2 = require('../planner');
  assert.ok(planner2.validateCrewPlan, 'sanity');
  // complexPlanSufficient 未导出,经 makePlan 路径验证成本高——直接验 prependMeeting forceComplex
  const withMeet = planner.prependMeeting(JSON.parse(JSON.stringify(p1)), roleMap, null, [], true);
  assert.ok(withMeet.meeting, '复杂判定(forceComplex)2 步也开会, 实际 meeting=' + JSON.stringify(!!withMeet.meeting));
  assert.ok(withMeet.steps.length > 2, '会议步已前置');
  // 不传 forceComplex:2 步保持旧行为不开会(简单任务)
  const noMeet = planner.prependMeeting(JSON.parse(JSON.stringify(p1)), roleMap, null, []);
  assert.ok(!noMeet.meeting, '无复杂标记时 2 步不开会(旧启发保留)');
});

test('runTask:规划抛异常/空计划都转 failed,不卡 planning(fire-and-forget 无人 catch)', async () => {
  const runner = require('../runner');
  // 规划抛异常 → failed
  const s1 = open(':memory:'); const id1 = s1.createTask('x');
  await runner.runTask(id1, { store: s1, onEvent: () => {}, runs: new Map(), makePlan: async () => { throw new Error('ENOENT 模拟'); } });
  assert.equal(s1.getTask(id1).status, 'failed', '规划抛异常应转 failed, 实际=' + s1.getTask(id1).status);
  assert.ok(s1.getTaskMsgs(id1).some((m) => m.text.includes('规划失败')), '应有规划失败提示');
  // 规划返回空计划 → failed
  const s2 = open(':memory:'); const id2 = s2.createTask('y');
  await runner.runTask(id2, { store: s2, onEvent: () => {}, runs: new Map(), makePlan: async () => ({ task: 'y', steps: [] }) });
  assert.equal(s2.getTask(id2).status, 'failed', '空计划应转 failed, 实际=' + s2.getTask(id2).status);
});
