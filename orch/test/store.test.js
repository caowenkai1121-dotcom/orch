...
test('新功能:启用/停用 agent(停用的不进规划器可选列表)', () => {
  const s = open(':memory:'); s.seed();
  assert.notEqual(s.listAgents().find((a) => a.id === 'codex').enabled, 0); // 默认启用
  s.setAgentEnabled('codex', false);
  assert.equal(s.listAgents().find((a) => a.id === 'codex').enabled, 0);    // 停用
  const enabled = s.listAgents().filter((a) => (a.kind || 'cli') === 'cli' && a.enabled !== 0).map((a) => a.id);
  assert.ok(!enabled.includes('codex') && enabled.includes('claude'));      // 规划器只见启用的
  s.setAgentEnabled('codex', true);
  assert.equal(s.listAgents().find((a) => a.id === 'codex').enabled, 1);    // 复启用
});
