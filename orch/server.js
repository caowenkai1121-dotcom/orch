...
// 优雅退出:systemd stop / restart(SIGTERM)与 Ctrl-C(SIGINT)时,先按 PID 杀掉所有在跑任务的 agent 子进程树,
// 否则它们成孤儿(继续烧 token、占端口、下次启动端口冲突)。清理后退出。
let _shuttingDown = false;
function gracefulExit(sig) {
  if (_shuttingDown) return; _shuttingDown = true;
  try { console.log('[' + sig + '] 正在清理在跑子进程后退出…'); } catch (e) {}
  try { runs.forEach((rec) => { if (rec && rec.children) rec.children.forEach((c) => { try { killTree(c); } catch (e) {} }); }); } catch (e) {}
  setTimeout(() => process.exit(0), 300).unref(); // 给 kill 一点时间,再退
}
['SIGTERM', 'SIGINT'].forEach((sig) => process.on(sig, () => gracefulExit(sig)));
...
app.get('/healthz', (req, res) => {
  try {
    const ts = store.listTasks(); // 顺带验证 DB 可读:真健康检查应验依赖,否则 DB 坏了还回 200 会瞒过监控
    const running = ts.filter((t) => t.status === 'running' || t.status === 'planning' || t.status === 'meeting').length;
    res.json({ ok: true, pid: process.pid, uptime: Math.round(process.uptime()), tasks: { running, total: ts.length }, ts: new Date().toISOString() });
  } catch (e) {
    res.status(500).json({ ok: false, error: 'db unavailable' }); // DB 不可读 → 探活失败,负载均衡/监控据此摘除或告警
  }
});
