...
- `engine.js` 核心调度:`runPlan` 依赖驱动(某步 deps 一满足即启动)+ 全局信号量并发上限 + 命名锁(`held` Set)。`runLoop` 质量门(body 末步 isGate,输出 PASS/FAIL,FAIL 重做≤max;冒泡 needDecision/needReplan 时同步 loop 步 blocked)。`runSession` 连续开发会话(`type:'session'` 步的全部 body 阶段在**同一个 CLI 进程**里一次跑完——开发思路/代码风格全程一致+免冷启动;流式【阶段完成:id】标记实时点亮画布 body 节点;超时按阶段数 timeoutScale 放大)。`runStep` 组装 brief 调 adapter.run;瞬时故障(网络/5xx)原地重试一次;model/effort 白名单清洗防命令行注入。
...
- `planner.js`:模板匹配 / LLM 拆分(`fromLLMRoles` 员工模式 / `fromLLM` 执行器模式);`sequentializeSteps`(拓扑重排+链式化,执行顺序=确认顺序)、`sanitizeDeps`(断环+去重id)、`lintPlan`(结构体检)、`extractJsonWithRepair`(JSON 坏回喂修复)、`resolveRoles`(role→agent+提示词+经验)、`prependMeeting`(复杂前置会议+智能选参会人;复杂判定即开会,不看步数)、`fallbackComplexRolePlan`(全栈保底编排)、`mergeEditedPlan`。**选人原则:能力对口不为用而用**。**拆步哲学:开发全在一个 CLI 会话**——LLM 按阶段/角色视角正常拆开发步(画布有节点),`foldDevSession` 自动把全部连续开发步折叠成一个 `type:'session'` 步,`runSession` 在同一个 CLI 进程里连续跑完(思路/风格一致+免冷启动);`isTestish` 判定的测试/验收/质量门排除在外,独立成步换人把关(实现者测自己有盲区)。session 展开同 loop(api relay/plan 按 body 节点显示,'开发会话'标签)。
